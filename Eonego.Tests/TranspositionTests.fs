...
/// sentinel, replacement/depth-preference, generation wrap, ttPv flag, and a concurrency stress test.
...
// ---------------------------------------------------------------------------
// Pack / unpack round-trip (incl. negative score/eval, full depth + 5-bit generation + ttPv bit).
// ---------------------------------------------------------------------------
[<Theory>]
...
let ``packData round-trips every field`` (move: int) (score: int) (eval: int) (depth: int) (bound: int) =
    let gen = 25 // 5-bit (0..31)
    let dPv = packData move score eval depth ((gen <<< 3) ||| (1 <<< 2) ||| bound)
    Assert.Equal(move, dMove dPv)
    Assert.Equal(score, dScore dPv)
    Assert.Equal(eval, dEval dPv)
    Assert.Equal(depth, dDepth dPv)
    Assert.Equal(bound, dBound dPv)
    Assert.Equal(gen, dGen dPv)
    Assert.True(dTtPv dPv) // pv bit set
    let dNoPv = packData move score eval depth ((gen <<< 3) ||| bound)
    Assert.False(dTtPv dNoPv) // pv bit clear
    Assert.Equal(bound, dBound dNoPv)
    Assert.Equal(gen, dGen dNoPv)
...
let ``store then probe round-trips`` () =
    ...
    let mv = 0x1ABC
    tt.Store key 9 BoundExact 250 -40 mv false
    let struct (hit, m, sc, ev, dp, bd, pv) = tt.Probe key
    Assert.True hit
    Assert.Equal(mv, m)
    Assert.Equal(250, sc)
    Assert.Equal(-40, ev)
...
let ``ttPv flag round-trips and is sticky across overwrites`` () =
    let tt = TranspositionTable(1)
    let key = 0x0123456789ABCDEFUL
    tt.Store key 9 BoundExact 250 -40 0x1ABC true
    let struct (_, _, _, _, _, _, pv1) = tt.Probe key
    Assert.True pv1
    // a later pv=false store over the SAME key must keep the pv mark (sticky).
    tt.Store key 9 BoundExact 251 -41 0x1ABC false
    let struct (_, _, _, _, _, _, pv2) = tt.Probe key
    Assert.True pv2

[<Fact>]
let ``probe of an absent key misses`` () =
    let tt = TranspositionTable(1)
    tt.Store 0xAAAAAAAAAAAAAAAAUL 5 BoundLower 10 0 0x111 false
    let struct (hit, _, _, _, _, _, _) = tt.Probe 0x5555555555555555UL
    Assert.False hit

[<Fact>]
let ``empty table never hits, including realKey = 0`` () =
    let tt = TranspositionTable(1)
    let struct (h0, _, _, _, _, _, _) = tt.Probe 0UL
    let struct (h1, _, _, _, _, _, _) = tt.Probe 0x1234UL
    Assert.False h0
    Assert.False h1
...
let ``injected torn read is rejected as a miss`` () =
    ...
    tt.Store key 6 BoundExact 123 0 0x222 false
    let struct (hit, _, _, _, _, _, _) = tt.Probe key
    Assert.True hit
    ...
    let struct (hit2, _, _, _, _, _, _) = tt.Probe key
    Assert.False hit2
...
let ``shallower non-exact store preserves a deeper entry's value but refreshes the move`` () =
    ...
    tt.Store key 8 BoundExact 100 0 0xAAA false
    tt.Store key 2 BoundLower 50 0 0xBBB false // depth+4 = 6 < 8 -> keep value/depth/bound, move updates
    let struct (_, m, sc, _, dp, bd, _) = tt.Probe key
    Assert.Equal(8, dp)
    Assert.Equal(100, sc)
    Assert.Equal(BoundExact, bd)
...
let ``deeper store overwrites value`` () =
    let tt = TranspositionTable(1)
    let key = 0x9E3779B97F4A7C15UL
    tt.Store key 2 BoundExact 30 0 0xAAA false
    tt.Store key 5 BoundLower 70 0 0xBBB false
    let struct (_, m, sc, _, dp, bd, _) = tt.Probe key
    Assert.Equal(5, dp)
    Assert.Equal(70, sc)
...
let ``storing MoveNone over the same position keeps the old move`` () =
    ...
    tt.Store key 5 BoundExact 10 0 0xABC false
    tt.Store key 6 BoundExact 20 0 MoveNone false
    let struct (_, m, _, _, dp, _, _) = tt.Probe key
    Assert.Equal(0xABC, m)
    Assert.Equal(6, dp)

// ---------------------------------------------------------------------------
// Generation 5-bit wrap (one bit was given to ttPv).
// ---------------------------------------------------------------------------
[<Fact>]
let ``NewSearch wraps the generation at 5 bits`` () =
    let tt = TranspositionTable(1)

    for _ in 1..200 do
        tt.NewSearch()

    Assert.InRange(tt.Generation, 0, 31)

[<Fact>]
let ``Clear empties the table`` () =
    let tt = TranspositionTable(1)
    let key = 0x1357913579135791UL
    tt.Store key 5 BoundExact 10 0 0x123 false
    tt.Clear()
    let struct (hit, _, _, _, _, _, _) = tt.Probe key
    Assert.False hit
...
let ``concurrent store and probe never crash and never validate a corrupt entry`` () =
    ...
    let work (tid: int) =
        ...
        for n in 1..perThread do
            ...
            if (n &&& 1) = 0 then
                tt.Store key (n &&& 63) BoundExact (int (int16 k)) 0 (int (uint16 k)) ((n &&& 2) = 0)
            else
                let struct (hit, _, sc, _, dp, _, _) = tt.Probe key
