/// Policy sidecar (Policy.fs) tests: EONPOL01 loader validation, kernel bit-exactness
...
// ---------------------------------------------------------------------------
// EONPOL01 byte-image builder (seeded random weights)
// ---------------------------------------------------------------------------
let private buildSidecar (seed: int) (ftHash: uint32) (withWdl: bool) : byte[] =
    let rng = Random(seed)
    use ms = new MemoryStream()
    use w = new BinaryWriter(ms)
    w.Write(Text.Encoding.ASCII.GetBytes "EONPOL01")
    w.Write(1) // version
    w.Write(int ftHash)
    w.Write(Hidden)
    w.Write(6) // shift0
    ...
    writeI32s SqOut 5000
    writeI8s (SqOut * Hidden)
    writeI32s SqOut 5000
    writeI8s (SqOut * Hidden)
...
let ``forwardFromFt is bit-exact across kernel paths`` () =
    ...
    for trial in 1..8 do
        ...
        let fScalar = Array.zeroCreate<int> 64
        let tScalar = Array.zeroCreate<int> 64
        ...
        if System.Runtime.Intrinsics.X86.Avx2.IsSupported then
            let fAvx = Array.zeroCreate<int> 64
            let tAvx = Array.zeroCreate<int> 64
        ...
        if System.Runtime.Intrinsics.X86.AvxVnni.IsSupported then
            let fVnni = Array.zeroCreate<int> 64
            let tVnni = Array.zeroCreate<int> 64
...
let ``fillLogits is deterministic and position-dependent`` () =
    ...
    | Some net ->
        ...
        let fA = Array.zeroCreate<int> 64
        let tA = Array.zeroCreate<int> 64
        let fB = Array.zeroCreate<int> 64
        let tB = Array.zeroCreate<int> 64
