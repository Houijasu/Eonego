...
/// Ensure a signature is solved, then run the independent verification pass over it (rebuilding the
/// promotion tables exactly as the solver does). None = proven correct; Some = first mismatch. Tests/tooling.
let internal verifySolved (pce: Piece) : string option =
    ensureSolved pce
    ...
    let promoTables =
        if pieceType pce = Pawn then
            let owner = pieceColor pce
            let t: sbyte[][] = Array.zeroCreate 6
            t.[Knight] <- solvedTable (makePiece owner Knight)
            t.[Bishop] <- solvedTable (makePiece owner Bishop)
            t.[Rook] <- solvedTable (makePiece owner Rook)
            t.[Queen] <- solvedTable (makePiece owner Queen)
            t
        else
            Array.empty

    verifySignature pce (solvedTable pce) promoTables
